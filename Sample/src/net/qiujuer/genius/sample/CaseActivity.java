...
public class CaseActivity extends Activity {
	...
	private void testToolKit() {
		// Synchronous mode in the main thread when operating the child thread
		// will enter the waiting,
		// until the main thread processing is completed
		// 同步模式在主线程操作时子线程将进入等待，直到主线程处理完成

		// Asynchronous mode the child thread parallel operation with the main
		// thread, don't depend on each other
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				// in this mode method first to execute commands on the queue,
				...
				// in this mode the child thread calls the method added to the
				// queue, can continue to go down, will not be blocked
				// 测试异步模式，在该模式下
	...
	private void testTool() {
		Log.i(TAG,
				"ToolKit：getAndroidId："
						+ Tools.getAndroidId(Genius.getApplication()));
	...
	private void testLog() {
		...
		// Whether open is written to the file, stored maximum number of files,
		// a single file size (Mb)
		// 是否开启写入文件，存储最大文件数量，单个文件大小（Mb）
		Log.setSaveLog(true, 10, 1);

		// Set whether to monitor external storage inserts
		// 设置是否监听外部存储插入操作
		// Open when inserting an external device (SD) to store the log file
		// copy to external storage devices
		// 开启时插入外部设备（SD）时将拷贝存储的日志文件到外部存储设备
		// This operation depends on whether written to the file open function,
		// the method is invalid when not open
	...
	private void testHashUtils() {
		Log.i(TAG,
				"HashUtils：QIUJUER's MD5：" + HashUtils.getMD5String("QIUJUER"));
	...
	private void testFixedList() {
		...
		addList.add(13);
		list.addAll(addList);
		Log.i(TAG, "FixedList:AddList:" + list.toString() + " " + list.size()
				+ " ," + list.getMaxSize());
		// Using poll pop-up elements
		// 采用poll方式弹出元素
		Log.i(TAG, "FixedList:Poll:[" + list.poll() + "] " + list.size() + " ,"
				+ list.getMaxSize());
		Log.i(TAG, "FixedList:Poll:[" + list.poll() + "] " + list.size() + " ,"
				+ list.getMaxSize());
		Log.i(TAG, "FixedList:Poll:[" + list.poll() + "] " + list.size() + " ,"
		...
		list.addLast(17);
		list.addLast(18);
		Log.i(TAG, "FixedList:AddLast:" + list.toString() + " " + list.size()
				+ " ," + list.getMaxSize());
		// From the head insert, delete the default tail beyond part
		// 从头部插入，默认删除尾部超出部分
		list.addFirst(19);
		list.addFirst(20);
		Log.i(TAG, "FixedList:AddFirst:" + list.toString() + " " + list.size()
				+ " ," + list.getMaxSize());

		// Clear
		// 清空操作
		list.clear();
		Log.i(TAG, "FixedList:Clear:" + list.toString() + " " + list.size()
				+ " ," + list.getMaxSize());
	...
	private void testCommand() {
		...
		Thread thread = new Thread() {
			public void run() {
				// The same way call way and the ProcessBuilder mass
				// participation
				// 调用方式与ProcessBuilder传参方式一样
				Command command = new Command(Command.TIMEOUT,
						"/system/bin/ping", "-c", "4", "-s", "100",
						"www.baidu.com");
		...
		// Async
		// 异步
		Command command = new Command("/system/bin/ping", "-c", "4", "-s",
				"100", "www.baidu.com");
		...
		Thread thread = new Thread() {
			public void run() {
				...
				try {
					// Add DNS service
					// 添加DNS服务器
					dns = new DnsResolve("www.baidu.com",
							InetAddress.getByName("202.96.128.166"));
					dns.start();
